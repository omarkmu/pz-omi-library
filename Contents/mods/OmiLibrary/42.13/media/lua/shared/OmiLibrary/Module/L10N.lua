...
---@type table<VanillaTranslationCategory, HashMap<string, string>>
---@private
L10N._vanillaTranslationMaps = {}
do
    local instance = Translator.new() -- only a little cursed
    ...
    local fieldToType = {
        ...
        itemEvolvedRecipeName = 'EvolvedRecipeName',
        recordedMedia = 'Recorded_Media',
    ...
    for i = 0, getNumClassFields(instance) - 1 do
        ...
        local name = tostring(field):match('([^%.]+)$')
        local category = fieldToType[name]
        if category then
            local map = getClassFieldVal(instance, field)
            if map and type(map) ~= 'string' then
                L10N._vanillaTranslationMaps[category] = map
            end
        end
    end
end
